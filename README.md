# 👋 Hi, I'm Evan (kh.kim)

친환경 기반 **AI**로 더 나은 세상을 꿈꾸는 공학자  
강원대학교 **정보통신공학과** · **스마트수소에너지 융합전공**

[![GitHub](https://img.shields.io/badge/GitHub-evan--kim--dev-181717?logo=github&logoColor=white)](https://github.com/evan-kim-dev)
[![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![Next.js](https://img.shields.io/badge/Next.js-000000?logo=nextdotjs&logoColor=white)](https://nextjs.org/)
[![FastAPI](https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white)](https://fastapi.tiangolo.com/)
[![AWS](https://img.shields.io/badge/AWS-232F3E?logo=amazonwebservices&logoColor=white)](https://aws.amazon.com/)
[![Flutter](https://img.shields.io/badge/Flutter-02569B?logo=flutter&logoColor=white)](https://flutter.dev/)

---

## 🌱 About Me

정보통신으로 **연결·시스템**을 배우고, 스마트수소에너지 융합으로 **친환경 에너지**를 익힌 뒤, 그 위에서 웹·모바일 AI 서비스를 API·데이터·배포까지 이어 붙입니다.

- **풀스택 + AI**: Next.js / Flutter 프론트와 FastAPI·Route Handlers 백엔드, Gemini·AWS Bedrock(RAG·Agent·Guardrails)을 실제 제품에 붙인 경험이 있습니다.
- **클라우드·운영**: DynamoDB·S3·EventBridge·ECS, Vercel·GitHub Actions까지 설계·배포·관측을 같이 봤습니다.
- **도메인**: 의료 정서 돌봄(Beddy), 시니어 디지털 안전, 생성형 AI 윤리 평가처럼 **안전·평가가 중요한** 문제를 선호합니다.

---

## 🎓 Education

**강원대학교** · 정보통신공학과 · 스마트수소에너지 융합전공

- **정보통신공학**: 통신·신호처리·네트워크·디지털 시스템 등 ICT 기반을 다루는 전공입니다. 소프트웨어만의 CS가 아니라, 시스템이 어떻게 연결되고 데이터를 주고받는지까지 설계하는 쪽에 무게가 있습니다.
- **스마트수소에너지 융합**: 수소 에너지의 생산·저장·운송·활용과 에너지 소재를 배우며, 데이터·AI를 수소 산업 현장에 붙이는 융합 과정입니다. 친환경·에너지 도메인 위에서 AI 서비스를 설계하는 제 방향과 맞닿아 있습니다.

---

## 🛠️ Tech Stack

| | |
|---|---|
| **Web / App** | TypeScript · Next.js · React · Flutter · Vite · Tailwind · Capacitor |
| **Backend** | FastAPI · Next.js Route Handlers · Prisma · Supabase · Convex · SQLite |
| **AI** | Gemini · OpenAI / OpenRouter · AWS Bedrock · LangChain · Chroma · RAG · Agent |
| **Cloud** | AWS (DynamoDB · S3 · EventBridge · ECS/ALB) · Vercel · GitHub Actions |

---

## 🏆 Awards

- 🥇 **대상** — AWS Healthcare AI Jumpstart: Prompthon, AWS Korea Public Sector, Korea · Aug. 2026
- 🥇 **대상** — AI Intensive Bootcamp, Kangwon National University, Korea · Jul. 2026
- 🥇 **대상** — X-Corps Research Festival, Kangwon National University, Korea · Nov. 2019
- 🥈 **은상** — The 3rd X-Corps Festival, National Research Foundation of Korea, Korea · Nov. 2019
- 🥈 **은상** — Capstone Design, Kangwon National University, Korea · Nov. 2019
- 🏅 **우수상** — Autumn Conference, The Korean Institute of Illuminating and Electrical Installation Engineers (KIIEE), Korea · Sep. 2019

---

## 🚀 Projects

### 🏥 Beddy — 장기입원 청소년 AI 정서 동반 + 보호자 요약 돌봄
[![Repo](https://img.shields.io/badge/GitHub-beddy-181717?logo=github)](https://github.com/evan-kim-dev/2026-aws-healthcare-ai-prompthon-beddy)
![AWS Bedrock](https://img.shields.io/badge/AWS%20Bedrock-FF9900?logo=amazonwebservices&logoColor=white)
![Guardrails](https://img.shields.io/badge/Guardrails-232F3E)
![Agent](https://img.shields.io/badge/Agent-0EA5E9)
![DynamoDB](https://img.shields.io/badge/DynamoDB-4053D6?logo=amazondynamodb&logoColor=white)

입원 청소년용 정서 동반 AI와 보호자용 요약 돌봄 서비스입니다.  
**TypeScript / Next.js**, AWS Bedrock(Converse·Guardrails·Agent), DynamoDB, S3, EventBridge.  
**배포**: AWS ECS Fargate + ALB.

🔗 [Repository](https://github.com/evan-kim-dev/2026-aws-healthcare-ai-prompthon-beddy)

---

### ✈️ Met U — 예산 기반 AI 여행 플래너
[![Repo](https://img.shields.io/badge/GitHub-MetU-181717?logo=github)](https://github.com/evan-kim-dev/MetU)
[![Demo](https://img.shields.io/badge/Demo-met--u.vercel.app-000000?logo=vercel)](https://met-u.vercel.app)
![Supabase](https://img.shields.io/badge/Supabase-3FCF8E?logo=supabase&logoColor=white)
![OpenRouter](https://img.shields.io/badge/OpenAI%20%2F%20OpenRouter-412991?logo=openai&logoColor=white)
![Hotelbeds](https://img.shields.io/badge/Hotelbeds-0A66C2)
![TourAPI](https://img.shields.io/badge/TourAPI-CD2E3A)

예산·일정에 맞춰 AI가 여행 플랜과 상품을 짜 주는 웹 서비스입니다.  
**TypeScript / Next.js**, **Python / FastAPI**, Supabase, OpenAI·OpenRouter, Hotelbeds·TourAPI.  
**배포**: Vercel (프론트) · Render (API). Flutter 클라: [Met-U](https://github.com/evan-kim-dev/Met-U).

🔗 [Repository](https://github.com/evan-kim-dev/MetU) · [Live Demo](https://met-u.vercel.app)

---

### 🗺️ potato — 강원 로컬 AI 관광 큐레이션
[![Repo](https://img.shields.io/badge/GitHub-potato-181717?logo=github)](https://github.com/evan-kim-dev/potato)
[![Demo](https://img.shields.io/badge/Demo-potato--peach.vercel.app-000000?logo=vercel)](https://potato-peach.vercel.app)
![Gemini](https://img.shields.io/badge/Gemini-8E75B2?logo=googlegemini&logoColor=white)
![Kakao Maps](https://img.shields.io/badge/Kakao%20Maps-FFCD00?logo=kakao&logoColor=000)
![TourAPI](https://img.shields.io/badge/TourAPI-CD2E3A)

강원 지역 스팟·코스를 AI로 추천하는 로컬 관광 웹입니다.  
**TypeScript / Next.js**, Gemini, Kakao Maps·Leaflet, TourAPI, Python(데이터 동기화).  
**배포**: Vercel · GitHub Actions로 TourAPI/지역 데이터 갱신.

🔗 [Repository](https://github.com/evan-kim-dev/potato) · [Live Demo](https://potato-peach.vercel.app)

---

### ⚖️ AI Ethics Evaluation — 생성형 AI 윤리 위험도 비교 MVP
[![Repo](https://img.shields.io/badge/GitHub-ai--ethics--evaluation-181717?logo=github)](https://github.com/evan-kim-dev/ai-ethics-evaluation)
![Gemini](https://img.shields.io/badge/Gemini-8E75B2?logo=googlegemini&logoColor=white)
![LLM as Judge](https://img.shields.io/badge/LLM--as--Judge-111827)
![SQLite](https://img.shields.io/badge/SQLite-003B57?logo=sqlite&logoColor=white)

프롬프트 조건별로 AI 답변의 윤리 위험도를 비교·채점하는 연구용 MVP입니다.  
**Python / FastAPI**, **TypeScript / React·Vite**, Gemini(LLM-as-Judge), SQLite.  
**배포**: 로컬·자체 서버 실행 중심 (연구 실험 도구).

🔗 [Repository](https://github.com/evan-kim-dev/ai-ethics-evaluation)

---

### 🛡️ senior_safe_portal — 시니어 디지털 보안관
[![Repo](https://img.shields.io/badge/GitHub-senior--safe--portal-181717?logo=github)](https://github.com/evan-kim-dev/senior_safe_portal)
[![Demo](https://img.shields.io/badge/Demo-GitHub%20Pages-222222?logo=githubpages)](https://rlarlgns-evan.github.io/senior_safe_portal/)
![Supabase](https://img.shields.io/badge/Supabase-3FCF8E?logo=supabase&logoColor=white)
![Edge Functions](https://img.shields.io/badge/Edge%20Functions-3ECF8E?logo=deno&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini-8E75B2?logo=googlegemini&logoColor=white)

시니어가 의심 링크·검색어를 넣고 위험 여부를 확인하는 포털입니다.  
**HTML / CSS / JavaScript**, Supabase Edge Functions, Gemini.  
**배포**: GitHub Pages · Edge Functions는 Supabase.

🔗 [Repository](https://github.com/evan-kim-dev/senior_safe_portal) · [Demo](https://rlarlgns-evan.github.io/senior_safe_portal/)

---

### 🪷 Kleshas — 불교 번뇌 테마 멘탈케어
[![Repo](https://img.shields.io/badge/GitHub-Kleshas-181717?logo=github)](https://github.com/evan-kim-dev/Kleshas)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C)
![Chroma](https://img.shields.io/badge/Chroma-FF6F61)
![Capacitor](https://img.shields.io/badge/Capacitor-119EFF?logo=capacitor&logoColor=white)

경전 RAG 챗과 수행·기록 기능이 있는 멘탈케어 앱입니다.  
**TypeScript / Next.js**, **Python / FastAPI**, Prisma·SQLite, LangChain·Chroma, Capacitor.  
**배포**: 웹은 Vercel/자체 호스팅, 모바일은 Capacitor 패키징.

🔗 [Repository](https://github.com/evan-kim-dev/Kleshas)

---

### 📈 market-news — 한·미 시장 브리핑 대시보드
[![Repo](https://img.shields.io/badge/GitHub-market--news-181717?logo=github)](https://github.com/evan-kim-dev/market-news)
![Convex](https://img.shields.io/badge/Convex-EE342F)
![Gemini](https://img.shields.io/badge/Gemini-8E75B2?logo=googlegemini&logoColor=white)
![TradingView](https://img.shields.io/badge/TradingView-2962FF?logo=tradingview&logoColor=white)

한·미 지수·뉴스를 모아 AI 일일 브리핑을 보여주는 대시보드입니다.  
**TypeScript / Next.js**, Convex, Gemini, Yahoo Finance·RSS, TradingView.  
**배포**: Vercel · Convex cron으로 수집.

🔗 [Repository](https://github.com/evan-kim-dev/market-news)

---

### 🧳 travel — Met U 축소/초기 버전
[![Repo](https://img.shields.io/badge/GitHub-travel-181717?logo=github)](https://github.com/evan-kim-dev/travel)
[![Demo](https://img.shields.io/badge/Demo-Vercel-000000?logo=vercel)](https://travel-evan-s.vercel.app)

Met U보다 앞서 만든 예산 AI 여행 웹의 초기 버전입니다.  
**TypeScript / Next.js**, **Python / FastAPI**, Supabase, Hotelbeds·TourAPI.  
**배포**: Vercel.

🔗 [Repository](https://github.com/evan-kim-dev/travel) · [Demo](https://travel-evan-s.vercel.app)

---

## 📫 Contact

GitHub · [@evan-kim-dev](https://github.com/evan-kim-dev)
